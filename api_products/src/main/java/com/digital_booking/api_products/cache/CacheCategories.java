...
public class CacheCategories {
    ...
    public void startCacheForCategories(){
        ...
        log.info("CATEGORIES-CACHE MEMORY: Se guardaron las categorias con ids: " + Arrays.asList(ids));
    ...
    public Category checkCacheForCategory(Long id) throws ResourceNotFound {
        ...
        if(category == null){
            category = categoryFeignClient.findCategoryById(id);
            if(category != null) this.cacheCategories.put(category.getId(), category);
        }
        return category;
    }
