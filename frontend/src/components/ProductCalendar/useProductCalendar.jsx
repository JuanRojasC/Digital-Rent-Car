...
export const useProductCalendar = (bookings) => {

	const moment = extendMoment(Moment);
	const {setRangeDate} =useContext(UserContext);
	const rangebookings = bookings.map((booking, i) => {
	...
	useEffect(() => {
		...
		while (dayLeft.isBefore(endDayLeft, "day")) {
			a.push( Array(7).fill(0).map(() => dayLeft.add(1, "day").clone()) );
		}
		while (dayRight.isBefore(endDayRight, "day")) {
			b.push (Array(7).fill(0).map(() => dayRight.add(1, "day").clone()) );
		}
		setCalendarLeft(a);
		setCalendarRight(b);
